...
function handsUp(kp) {
  ...
  for (const [wrist, elbow] of checks) {
    if (confident(wrist, elbow) && wrist.y >= elbow.y) return false; // this wrist is at/below elbow — not guarding
  }
  return true;
}
...
export class MoveDetector {
  ...
  _lateralArc(kp, det) {
    ...
    if (confident(oWrist, oElbow) && oWrist.y >= oElbow.y) return 0; // off-hand dropped — not guarding

...
export class MoveDetector {
  ...
  _wristExtension(kp, det) {
    ...
    // FAIL-CLOSED: if we can't clearly confirm the off-hand is up, treat it
    // as NOT guarding rather than assuming it's fine. Punches require
    // visible proof of the guard, not just absence of evidence against it.
  ...
  _lateralArc(kp, det) {
    ...
    if (!confident(oWrist, oElbow) || oWrist.y >= oElbow.y) return 0; // off-hand dropped or unverifiable — not guarding

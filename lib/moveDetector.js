...
const MIN_SCORE = 0.15; // ignore keypoints the model isn't confident about — lowered so fast-punch motion blur (which drags confidence down on real, valid detections) doesn't discard the peak-extension frame
...
// Guard check for defensive movements (slips, bob & weave) — both wrists
// must be visibly above their respective elbows, simulating hands held up
// near the face/temples rather than down at the sides. If either wrist
// isn't confidently tracked, this fails open (returns true) rather than
// blocking detection entirely on a brief tracking dropout — confidence
// gaps on individual keypoints are common and shouldn't punish form that
// might actually be correct.
function handsUp(kp) {
  const checks = [
    [kp.left_wrist, kp.left_elbow],
    [kp.right_wrist, kp.right_elbow],
  ];
  for (const [wrist, elbow] of checks) {
    // FAIL-CLOSED: an unconfident/unclear reading is treated as NOT
    // guarding, not assumed fine — matches the punch off-hand guards.
    if (!confident(wrist, elbow) || wrist.y >= elbow.y) return false;
  }
  return true;
}
...
export class MoveDetector {
  ...
  update(kp) {
    ...
    const MIN_RISE = 0.15;     // how much higher than the recent baseline counts as a real rise (lowered — 0.22 was rejecting clean reps)
  ...
  _computeMetric(kp) {
    ...
    switch (det.method) {
      ...
      case 'knee_raise':            return this._kneeRaise(kp);
  ...
  _wristExtension(kp, det) {
    ...
    // Off-hand guard: the differential check below compares arm straightness,
    // but a bent-but-not-raised off-arm (e.g. holding something at waist
    // height) can still pass that check while not actually guarding. This
    // explicit check requires the off-hand to be ABOVE its own elbow,
    // same as the guard used for hooks/uppercuts/slips.
    const oWristGuard = kp[`${otherSide}_wrist`], oElbowGuard = kp[`${otherSide}_elbow`];
    // FAIL-OPEN: only reject when we can CLEARLY see the off-hand is dropped.
    // A tracking dropout on the off-hand (common during fast motion) shouldn't
    // kill a clean punch — the differential check below still catches both
    // arms moving together whenever both arms are confidently visible.
    if (confident(oWristGuard, oElbowGuard) && oWristGuard.y >= oElbowGuard.y) return 0;
    ...
    if (confident(oShoulder, oElbow, oWrist)) {
      ...
      if (oAng !== null) {
        ...
        if (targetMetric - otherMetric < 0.15) return 0; // both arms moving together — not a clean single punch
      }
    }
    return targetMetric;
  }
  ...
  // The OFF-hand (not throwing the hook) must stay up guarding the face —
  // same concept as the slip guard, just only checking the one arm that
  // isn't actively punching.
  _lateralArc(kp, det) {
    const isLead = det.primary.includes('lead');
    const side = resolveSide(isLead ? 'lead' : 'rear', this.stance);
    const otherSide = side === 'left' ? 'right' : 'left';
    ...
    // Off-hand guard check
    const oWrist = kp[`${otherSide}_wrist`], oElbow = kp[`${otherSide}_elbow`];
    if (confident(oWrist, oElbow) && oWrist.y >= oElbow.y) return 0; // off-hand clearly dropped — not guarding (fail-open on a tracking dropout)
    ...
    if (elbowAngle !== null && elbowAngle > 158) return 0; // fully straight — that's a cross, not a hook
    const lateralOffset = Math.abs(wrist.x - shoulder.x) / scale;
    // Loosened from (lateralOffset - 0.3) / 0.8 — was demanding more lateral
    // travel than most clean, compact hooks actually produce.
    return Math.max(0, Math.min(1, (lateralOffset - 0.18) / 0.55));
  }

  // Uppercuts — wrist rises sharply relative to the shoulder line.
  // Uppercuts — wrist rises sharply relative to the shoulder line.
  // The off-hand (not throwing the uppercut) must stay up guarding.
  _upwardArc(kp, det) {
    const isLead = det.primary.includes('lead');
    const side = resolveSide(isLead ? 'lead' : 'rear', this.stance);
    const otherSide = side === 'left' ? 'right' : 'left';
  ...
  //
  // IMPORTANT: the camera faces the boxer, so the boxer's own right side
  // appears on the LEFT side of the image (same as facing someone directly
  // — their right hand lines up with your left). MoveNet returns raw image
  // coordinates, not person-corrected ones, so detecting the boxer's own
  // "right" requires checking for a NEGATIVE x-offset (image-left), not
  // positive — the inverse of what you'd naively expect.
  _headLateral(kp, det) {
    const nose = kp.nose, lh = kp.left_hip, rh = kp.right_hip;
    const scale = bodyScale(kp);
    if (!confident(nose, lh, rh) || !scale) return null;
    if (!handsUp(kp)) return 0; // hands must be up guarding — not a proper slip otherwise
    const centerX = (lh.x + rh.x) / 2;
    const offset = (nose.x - centerX) / scale;
    // Flipped vs. naive mapping — see comment above.
  ...
  _headVerticalLateral(kp) {
    ...
    if (!handsUp(kp)) return 0; // hands must be up guarding
  ...
  //
  // BUG FIX: previously used the live hip-to-knee gap itself as the scale
  // reference, which collapses toward zero at exactly the same time as
  // the numerator (the deeper you squat, the smaller BOTH become), making
  // the ratio unstable right when it mattered most — this is why squats
  // weren't registering at all, even very deep ones. Now uses shoulder
  // width as a fixed reference that doesn't change as you move.
  _hipDepth(kp) {
    const { left_hip: lh, right_hip: rh, left_knee: lk, right_knee: rk } = kp;
    const scale = bodyScale(kp);
    if (!confident(lh, rh, lk, rk) || !scale) return null;
    const hipY = (lh.y + rh.y) / 2, kneeY = (lk.y + rk.y) / 2;
    // Standing: hip well above knee — gap is large relative to shoulder width.
    // Deep squat: hip drops toward knee height — gap shrinks toward ~0.
    const gap = (kneeY - hipY) / scale;
    // Standing gap ~1.6x shoulder width -> metric 0. Deep squat gap ~0.5x -> metric 1.
    return Math.max(0, Math.min(1, (1.6 - gap) / 1.1));
  }

  // Broad jump — both ankles rise together off their resting ground baseline.
  // Knee raise — either knee lifted above hip height counts, doesn't
  // matter which leg. Uses whichever knee is more confidently tracked
  // and currently higher, since only one leg is typically raised at a time.
  _kneeRaise(kp) {
    const { left_hip: lh, right_hip: rh, left_knee: lk, right_knee: rk } = kp;
    const scale = bodyScale(kp);
    if (!confident(lh, rh) || !scale) return null;

    const hipY = (lh.y + rh.y) / 2;
    let bestRise = null;
    if (confident(lk)) {
      const rise = (hipY - lk.y) / scale; // positive = this knee is above hip height
      bestRise = bestRise === null ? rise : Math.max(bestRise, rise);
    }
    if (confident(rk)) {
      const rise = (hipY - rk.y) / scale;
      bestRise = bestRise === null ? rise : Math.max(bestRise, rise);
    }
    if (bestRise === null) return null;

    // Standing (knee well below hip) -> metric 0. Knee raised above hip -> metric 1.
    return Math.max(0, Math.min(1, (bestRise + 0.3) / 0.6));
  }
